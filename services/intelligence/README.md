...
| `INTELLIGENCE_PORT`   | `8888`      | uvicorn bind port           |
...
Startup registers the `inquiry` and `attachment-interpretation` runners. An unknown graph name returns 404
(or a `run:error` SSE event).
